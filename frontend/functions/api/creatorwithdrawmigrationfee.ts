// src/pages/api/creatorWithdrawMigrationFee.ts
...
export const onRequestPost: PagesFunction<ENV> = async (ctx) => {
  ...
  try {
    ...
    try {
      ...
      // Verify the sender is the creator (poolCreator) of the virtual pool
      ...
      console.log("Creator authorization verified:", poolInfo.creator.toBase58());
      ...
      // Get pool config to check migration fee settings
      const configInfo = await client.state.getPoolConfig(poolInfo.config);
      console.log("Migration fee configuration found for config:", poolInfo.config.toBase58());
    ...
    const withdrawTx = await client.creator.creatorWithdrawMigrationFee({
      ...
      feePayer: creatorWallet, // Optional: defaults to sender if not provided
    });

    console.log("Creator withdraw migration fee transaction created");
    ...
    console.log("Creator withdraw migration fee transaction signature:", txSignature);
    ...
    try {
      await db
        ...
        .bind(finalPoolAddress, creatorWallet.toBase58(), txSignature, 'migration_fee_creator')
        .run();
      console.log("Creator migration fee withdrawal logged to database");
    } catch (dbError) {
    ...
    return new Response(JSON.stringify({
      ...
      withdrawalType: 'migration_fee_creator',
      note: "Creator's migration fee has been withdrawn successfully"
    }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
    ...
    console.error('Creator withdraw migration fee error:', error);
    ...
    if (error instanceof Error) {
      if (error.message.includes('Account does not exist')) {
        ...
        errorMessage = 'Only the pool creator can withdraw creator migration fees';
      } else {
